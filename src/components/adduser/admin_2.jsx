...
function Admin_2() {
    ...
    useEffect(() => {
      fetchData();
      const subscription = supabase.from('staff_detils').on('*', () => fetchData()).subscribe();
    ...
    return (
      ...
      {data.map((item, index) => (
        <div key={index} className='w-full h-[48px] bg-[rgba(0,0,0,0)] relative z-[2] mt-0 mr-0 mb-0 ml-0'>
